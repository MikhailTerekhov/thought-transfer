...
enroot import docker://registry.rcp.epfl.ch#claire/moalla/thought-transfer:amd64-cuda-root-latest
...
# Don't do this on a login node.
salloc --exclusive --partition h100 --time=4:00:00 \
apptainer pull docker://registry.rcp.epfl.ch/claire/moalla/thought-transfer:amd64-cuda-root-latest
```
...
```bash
cd $SCRATCH/thought-transfer/dev/
...
git pull
cd thought-transfer/dev/installation/docker-amd64-cuda
```
...
3. Then follow the instructions [here](https://www.jetbrains.com/help/pycharm/remote-development-a.html#gateway) and
   ...
   something like `/scratch/moalla/thought-transfer/dev`).
