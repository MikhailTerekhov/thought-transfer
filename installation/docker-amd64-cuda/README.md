...
8. Push your generic image (the one with the root user) to some registry if not done already.
   This will be handy for you, for sharing it with your teammates, and when you open-source your project later.
    ...
    3. Add this link to the TODO ADD PULL_IMAGE_NAME in
       ...
       e.g., ic-registry.epfl.ch/your-lab/your-gaspar/thought-transfer_.)
...
```bash
git clone <HTTPS/SSH> thought-transfer
cd thought-transfer
```
...
./template.sh run python -m thought_transfer.template_experiment some_arg=some_value
...
```bash
mv thought-transfer thought-transfer-tmp
mkdir thought-transfer
mv thought-transfer-tmp thought-transfer/dev
...
E.g., you can mount it at `/project/thought-transfer` and specify `PROJECT_ROOT_AT=/project/thought-transfer`.
...
export PROJECT_ROOT_AT=/project/thought-transfer
apptainer run \
    -c \
    ...
    --env WANDB_API_KEY="" \
    --nv thought-transfer_amd64-cuda-root-latest.sif
